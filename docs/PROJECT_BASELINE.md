...
- `CURRENT_ARCHITECTURE.md` is a pre-refactor snapshot. It says structured party/budget/pace preferences, evidence models, validation, patch/version workflows, LLM usage observation, and frontend polling fallback do not exist. They now exist in schemas/services/UI/tests. Its warnings at lines/sections discussing raw secret return are also outdated: current settings responses expose presence flags and browser-safe values, not backend keys/cookies.
- `CURRENT_ARCHITECTURE.md` says AMap service parsing remains TODO and generic APIs may return empty structures. Current `amap_service.py` has REST parsing and extensive tests, although live provider correctness remains unverified.
...
The strongest boundary is operational: the repository proves implementation and deployment configuration, not a healthy production service or valid external integrations. It is deliberately single-process, has no authentication or database/job queue, cannot resume in-flight tasks after restart, and provides partial rather than universal evidence grounding. `README.md` mostly reflects the current implementation, while `CURRENT_ARCHITECTURE.md` materially predates the Phase 1–5 work and should not be used as the current source of truth without this baseline.
